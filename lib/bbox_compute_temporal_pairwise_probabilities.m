...
% build spatial edges with-in frames.
for f = 1:num_frames
    fIdx = frames(f);
    num_dets  = sum(detections.frameIndex == fIdx);
    spatial_det_rel{f} = bbox_build_frame_pairs(num_dets, num_dets); %FIXME: change the name of the function to make it general.
...
for f = 1:num_frames
    ...
    if p.locref
        ...
        if(num_dets == 1)
            ...
            % bboxs' dets.unPos is after Refinement.
    ...
    if(size(locations,2) == 2)
        ...
        x1 = locations(:,1) - r;
        y1 = locations(:,2) - r ;
        x2 = x1 + r;    %TODO: wrong? ----jinsheng
        y2 = y1 + r; 
        bbox = cat(2, x1,y1,x2,y2);
    else
        % locations(:, 3) = locations(:, 3) + locations(:, 1);
        % locations(:, 4) = locations(:, 4) + locations(:, 2);
        bbox = locations;
    end
...
for f = 1:size(frame_pairs,1)    
    ...
    corres_pts = bbox_load_dm_correspondences(corres_fn);
