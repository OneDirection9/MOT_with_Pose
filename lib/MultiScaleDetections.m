...
methods(Static)
    ...
    function dets = merge(dets1, dets2)
        ...
        if isfield(dets1, 'unPos_sm')
            if ~isfield(dets2, 'unPos_sm')
                dets2.unPos_sm = NaN(size(dets2.unPos, 1), 2);
            end
            dets.unPos_sm = cat(1, dets1.unPos_sm, dets2.unPos_sm);
        end
        if isfield(dets1, 'locationRefine')
            if ~isfield(dets2, 'locationRefine')
                dets2.locationRefine = NaN(size(dets2.unPos, 1), 1, 2);
            end
            dets.locationRefine = cat(1, dets1.locationRefine, dets2.locationRefine);
        end
        if isfield(dets1, 'nextReg')
            if ~isfield(dets2, 'nextReg')
                dets2.nextReg = NaN(size(dets2.unPos, 1), 182, 2);
            end
            dets.nextReg = cat(1, dets1.nextReg, dets2.nextReg);
        end
        if isfield(dets1, 'unProbNoThresh')
            if ~isfield(dets2, 'unProbNoThresh')
                dets2.unProbNoThresh = NaN(size(dets2.unPos, 1), 1);
            end
